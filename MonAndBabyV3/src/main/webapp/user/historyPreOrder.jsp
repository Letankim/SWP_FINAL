                ...
                <table class="table-responsive table table-hover" id="history">
                    ...
                    <tbody>
                        <c:forEach items="${preOrders}" var="preOrder" varStatus="status">
                            ...
                            <c:if test="${productPreOrder != nulll && accountPreOrder != null}">
                                <tr>
                                    <td>${status.index + 1}</td>
                                    <td>${accountPreOrder.username}</td>
                                    <td>
                                        <a href="/MomAndBaby/product/detail/${productPreOrder.ID}">${productPreOrder.name}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a onclick="return confirmDelete(${preOrder.ID})" href="/MomAndBaby/account/pre-order/delete/${preOrder.ID}" title="Delete">
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const url = "/MomAndBaby/account/pre-order/delete/" + id;
