...
public class BlogController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/MomAndBaby/staff/blog")) {
            this.getAllBlog(request, response);
        } else {
            String paths[] = path.split("/");
            int id = validate.getInt(paths[paths.length - 1]);
            if (path.startsWith("/MomAndBaby/staff/blog/view")) {
                this.getCurrentBlog(request, response, id);
            } else if (path.startsWith("/MomAndBaby/staff/blog/delete")) {
//                this.delete(request, response, id);
            } else if (path.startsWith("/MomAndBaby/staff/blog/update")) {
    ...
    public void addNew(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (nameImgBlog != null) {
                ...
                response.sendRedirect("/MomAndBaby/staff/blog?act=add-blog&status=" + result);
            } else {
                response.sendRedirect("/MomAndBaby/staff/blog?act=add-blog&status=2");
            }
        } catch (Exception e) {
    ...
    public void updateBlog(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            response.sendRedirect("/MomAndBaby/staff/blog?act=update-blog&status=" + result);
        } catch (Exception e) {
