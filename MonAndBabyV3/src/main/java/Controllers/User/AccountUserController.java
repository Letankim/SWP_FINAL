...
public class AccountUserController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accountLogin != null && accountLogin.getStatus() == 1) {
            ...
            } else if (path.endsWith("/MomAndBaby/account/pre-order")) {
            ...
            } else if (path.startsWith("/MomAndBaby/account/pre-order/delete")) {
                PreOrderDAO preOrderDao = new PreOrderDAO();
                String paths[] = path.split("/");
                int idPreOrder = validate.getInt(paths[paths.length - 1]);
                int result = preOrderDao.delete(idPreOrder);
                response.sendRedirect("/MomAndBaby/account/pre-order?act=delete-preOrder&status=" + result);
            } else if (path.startsWith("/MomAndBaby/account/history-order/detail")) {
                String paths[] = path.split("/");
            ...
            } else if (path.startsWith("/MomAndBaby/account/history-order/feedback")) {
                String paths[] = path.split("/");
                BillDetailDAO billDetailDao = new BillDetailDAO();
                int idOrder = validate.getInt(paths[paths.length - 1]);
                Bill bill = billDao.getBillByID(idOrder);
            ...
            if (request.getParameter("btn-change-info") != null) {
                ...
                if (!email.equals("")) {
                    ...
                    if (isExistEmail != null) {
                        if (isExistEmail.getUsername().toLowerCase().equals(accountLogin.getUsername().toLowerCase())) {
                            isValidEmail = true;
                        }
                    }
                }
                if (isValidEmail) {
                    ...
                    response.sendRedirect("/MomAndBaby/account?act=update-account&status=" + type_message);
                } else {
                    response.sendRedirect("/MomAndBaby/account?act=update-account&status=2");
                }
                ...
                if (accountLogin.getPassword().equals(oldPassword)) {
                    ...
                    response.sendRedirect("/MomAndBaby/account/password?act=update-account&status=" + result);
                } else {
                    response.sendRedirect("/MomAndBaby/account/password?act=update-password&status=2");
                }
            } else {
                ...
                if (request.getParameter("btn-edit-feedback") != null) {
                    ...
                    response.sendRedirect("/MomAndBaby/account/history-order/feedback/" + idOrder + "?act=edit-feedback&status=" + result);
                } else if (request.getParameter("btn-new-feedback") != null) {
                    result = feedbackDao.add(feedback);
                    response.sendRedirect("/MomAndBaby/account/history-order/feedback/" + idOrder + "?act=feedback&status=" + result);
                }
            }
        }
    }
