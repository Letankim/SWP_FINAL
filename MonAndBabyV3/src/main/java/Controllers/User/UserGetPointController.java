    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (currentBill != null && currentBill.getStatus() == 5 && currentBill.getIsGetPoint() == 0) {
            ...
            if (resultChangeStatusPoint >= 1) {
                ...
                response.sendRedirect("/MomAndBaby/account/history-order/detail/" + idBill + "?act=get-point&status=" + result);
            } else {
                response.sendRedirect("/MomAndBaby/account/history-order/detail/" + idBill + "?act=get-point&status=" + 0);
            }
        } else {
            response.sendRedirect("/MomAndBaby/account/history-order/detail/" + idBill + "?act=get-point&status=2");
        }
    }
